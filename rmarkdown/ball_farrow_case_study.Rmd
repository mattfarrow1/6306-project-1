...
#distribution of ABV and IBU before imputation
hist(beer_brew$abv)
hist(beer_brew$ibu)
...
beer_brew <- beer_brew %>% 
  group_by(style) %>% 
  mutate_at(vars(abv), impute_mean) %>% 
  ungroup()
...
#still a stubborn 10 rows that are NA - this is because these styles don't have IBU. Remove these rows
beer_brew <- beer_brew %>% 
  drop_na(ibu)
...
#this code was grouping several states together as "NA" due to join issues
